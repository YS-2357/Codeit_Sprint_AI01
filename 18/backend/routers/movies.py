...
from database import fetch_all_movies, insert_movie, delete_movie, update_movie
...
async def delete_movie_api(movie: Movie):
    logger.debug(f"🗑️ 삭제 요청: {movie}")
    deleted = delete_movie(movie)
    if deleted == 0:
        logger.warning(f"⚠️ 삭제 실패 - 대상 없음: {movie.title} by {movie.director}")
        raise HTTPException(status_code=404, detail="해당 영화가 존재하지 않습니다.")
    logger.info(f"✅ 영화 삭제 완료: {movie.title}")
    return {"message": f"{movie.title} 영화가 삭제되었습니다."}